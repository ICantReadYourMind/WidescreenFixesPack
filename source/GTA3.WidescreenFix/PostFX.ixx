		...
		WFP::onInitEvent() += []()
			{
				...
				{
					auto pattern = hook::pattern("A3 ? ? ? ? A1 ? ? ? ? 3B C1");
					pBackBufferFormat = *pattern.get_first<D3DFORMAT*>(1);

					static auto D3D8SystemStartHook = safetyhook::create_mid(hook::pattern("A3 ? ? ? ? 5F 5E 5B 81 C4").get_first(), +[](SafetyHookContext& regs)
						{
							InitShaderAndStaticResources();
						});
...
private:
	...
	static inline D3DFORMAT* pBackBufferFormat = nullptr;
	...
	static void InitShaderAndStaticResources()
	{
		...
		HRESULT hr = D3DXCreateEffectFromResource(dev.Get(), hModule, MAKEINTRESOURCE(IDR_POSTFX),
		...
		if (SUCCEEDED(hrArea))
		{
			hrArea = D3DXCreateTextureFromResourceEx(dev.Get(), hModule, MAKEINTRESOURCE(IDR_AREATEX),
		...
		{
			hrSearch = D3DXCreateTextureFromResourceEx(dev.Get(), hModule, MAKEINTRESOURCE(IDR_SEARCHTEX),
	...
	static bool CreateTextures(IDirect3DDevice9* dev)
	{
		...
		if (FAILED(dev->CreateTexture(nScreenWidth, nScreenHeight, 1,
			D3DUSAGE_RENDERTARGET, *pBackBufferFormat, D3DPOOL_DEFAULT, &pSceneTex, nullptr)))
			return false;
		...
		if (SelectedMultisamplingLevels > 1) {
			if (FAILED(dev->CreateRenderTarget(nScreenWidth, nScreenHeight, *pBackBufferFormat,
